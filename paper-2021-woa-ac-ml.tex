...
One of the most challenging fields for Machine Learning application is Collective-Adaptive System (CAS). % Or do we want to use Cyber-Physical Swarm??
...
Nowadays, computer engineers have to deal with Collective Adaptive Systems (CASs)
 ...
 Examples of those include robot swarm, smart cities and augmented crowds with smart devices.
...
Here, a sort of hive mind arises -- 
 referred also as swarm intelligence -- 
...
Initially, researchers try to bring those properties into the computer science world 
 by miming the collective behaviour bottom-up~\cite{DBLP:journals/connection/Webb02}, 
...
In Aggregate Computing there are common abstractions in collective behaviour specification. 
 These are encapsulated in so-called ``building blocks''.
%
...
Though, building block synthesis concerns different stuff related not only to the behaviour itself
...
Ideally, we would continue to build our application using these building blocks -- so the high-level specification remains the same -- using some
...
To a more in depth explanation of these opearatos, please refear to~\cite{DBLP:journals/computer/BealPV15}.
...
Here we outline the different perspectives about Aggregate Computing with learning, for
 of clarifying the current and the future research path.
%
...
On the other side, learning can be useful also for framework related stuff,
 ...
 Trust mechanisms, evaluation frequency, energy consumption manager, package storage are all the things that currently have been defined by hand.
...
%
In Aggregate Computing, Supervised Learning can be applied when we want to craft new building blocks by zero. 
% 
...
Speaking by Reinforcement Learning settings, we need to define the state function and the reward function.
%
...
Towards a complete integration between Aggregate Computing and RL, we decide to set up an Independent learning technique, 
 by which each agent try to improve our current local policy (shown in \Cref{fig:aggregate-q-learning}) inspired by other work in CSAS~\cite{csas-and-marl} and swarm systems~\cite{nguyen2018swarm}.
...
At the end of the episodes, each agent has collected its local experience, which can vary in different zones. So, 
 we combine the N Q table selecting the maximum value for each state-action pair since we assume a homogeneous behaviour, 
 where each agent perform in the same way when reaching the same state.:
$$
...
Here, the computational field that will eventually be produced consists of the distance in terms of the minimum number of nodes needed to reach source nodes ((\Cref{fig:hop-count})).
...
So, our first intuition is to accelerate -- already done algorithmically -- of convergence through learning.
...
\begin{figure}
  ...
  performs periodically the standard hop count. 
  Using the output produced and the history of state and output, 
  each agent computes its current state. 
  Using it, they evaluate the action to apply to speed up the hop count convergence following a policy. 
...
We verify our learning algorithm through simulation~\footnote{repository at: \url{https://github.com/cric96/alpaca-2022-ac-rl}}.
...
We use 80 ($N$) nodes displaced into an irregular grid with a width of 100 meters and a height of 20 meters. 
 ...
 Each simulation episode least 120-time unit.
% 
...
\Cref{fig:simulation}-a shows the error for both naive implementation and Reinforcement Learning based implementation as time changes.
...
Then, we test our learnt algorithm in different environments (\Cref{fig:simulation} b,c,d). To do that, we share the Q-table learn to the 
entire aggregate and we test the program changing the system size (80, 360 and 760) nodes.
For each population size, we run 50 episodes and we average the error at each time step. As results, 
 the aggregate speed-up convergence in each new situation.
