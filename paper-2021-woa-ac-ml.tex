...
\sloppy
\begin{document}
\title{Towards a Reinforcement Learning approach for Aggregate Computing} %% or Towards a Machine Learning approach for Aggregate Computing
\author{
\IEEEauthorblockN{
...
%
For this reason, over the years, various approaches have addressed the problem of dealing with this complexity---for instance, by taking inspiration from natural systems like insect and animal societies (e.g., ant colonies, beehives, flocks of birds etc.). 
%
...
Initially, researchers had tried to build systems that show these properties in computing systems by mimicking natural collective behaviours ``from the bottom up''~\cite{DBLP:journals/connection/Webb02}, 
 namely trying to find the right single-node-behaviour (local rules)
 to achieve the intended global behaviour. %, e.g. flocking, foraging, etc.
%
...
%, abstracting over non-functional aspects such as the network topology, environment uncertainties and system size.
In this direction, Aggregate Computing (AC)~\cite{DBLP:journals/computer/BealPV15} is proposed:
 it is a paradigm by which
 developers can express collective self-organising behaviours from a global perspective. % through a functional program specification.
%
...
However, building block synthesis concerns different elements not only related to the behaviour itself
 but also to the ``ensemble" specifications. 
%
For instance, it is quite difficult to craft building blocks
 that easily adapt to the non-stationarity due to node movements.
%
Ideally, we would like to continue developing our applications using these building blocks 
...
%
For this reason, we claim that Machine Learning can help us in handling this "unseen" situation, leaving only the burden of defining the right collective specifications to developers.
...
%
In \Cref{background} we describe
 the AC framework, %showing the context in which is typically applied and the overall idea of how it works, and then the motivation of injecting Reinforcement Learning in this paradigm,
 and motivate our proposal to inject RL into this paradigm. 
 %in \Cref{aggregate-and-machine-learning} we detail the relevant properties of Aggregate Computing in terms of machine learning problems, 
 In \Cref{aggregate-and-rl}, we describe the details of our integration of AC with RL.
Then, in  \Cref{evaluation} we evaluate the approach on a paradigmatic AC case study.
%
Finally, in \Cref{conclusion} we discuss the results and the path towards deeper AC-RL integration.
...
Collective Adaptive Systems are characterized by a very high number of nodes without a central authority that coordinate nodes. 
 The overall behaviour is a consequence of local interactions.%The systems expose self-* properties.
...
Instead, a natural way to describe this kind of problems should be through a \textit{collective} view of the system.
 This is the idea used by the so-called \textit{global-to-local} techniques~\cite{DBLP:journals/jlap/ViroliBDACP19,DBLP:journals/scp/AlrahmanNL20, DBLP:conf/cbse/BuresGHKKP13}, 
 where the behaviours are designed \textit{top-down}, and then the local one is generated automatically from this ``system-wide'' specification. 
% 
...
The latter could be seen as the application of the mathematical concept of field: it consists of a 
...
%
The novelty of AC resides in its composable (functional-like) system-size independent program definition: 
 using basic constructs we identify the main patterns (\textit{building-blocks}) that could be reusable at application level.
...
AC is studied in various simulated scenarios ranging from smart cities to crowd management~\cite{DBLP:journals/jlap/ViroliBDACP19}, 
 ...
 this paradigm~\cite{DBLP:journals/jlap/ViroliBDACP19}, such as the ScaFi aggregate programming language~\cite{DBLP:conf/isola/CasadeiVAD20}. %, FCCP~\cite{DBLP:conf/acsos/Audrito20} and Protelis~\cite{PianiniSAC2015}.

In the next sections, we summarise the field calculus language and computation model, which are essential to understand the case study of \Cref{evaluation}.
...
\begin{itemize}
  ...
  For instance, \mintinline{scala}{minHood} returns the minimum value 
...
\end{itemize}
%
A full coverage of field calculus is beyond the scope of this paper. A more comprehensive presentation is given in~\cite{DBLP:journals/jlap/ViroliBDACP19}.
...
Consider the problem of associating
 ...
 from that node to the closest \emph{source} node (see \Cref{fig:hop-count}).
%
...
%%
% For example, a well-tested aggregate application in an environment with limited dynamics could lead to a non-stable behaviour under continuous perturbations.
%% 
...
%
Another way to handle multi-agent learning is to use the so-called \textit{Centralised Training and Decentralised Execution} (CTDE) algorithms. 
...
From a local point of view, AC program execution can be seen as:
$$
...
As far as RL settings are concerned, we need to define the state function and the reward function.
%
...
Instead, the reward function might be dependent on the learning technique, time, and problem. 
...
$$
As remarked before, the policy $\pi$ needs to be distributed, so that each agent can choose the action that will be performed:
$$
\textit{Action} \leftarrow \pi(K)
$$

As a first step towards a complete integration between AC and RL, we have decided to adopt an Independent RL learning technique, 
 by which each agent try to improve their own current local policy (shown in \Cref{fig:aggregate-q-learning}). 
% 
...
At the end of each episode, each agent collects its local experience, which can vary in different zones. 
...
\begin{minted}[fontsize=\scriptsize,mathescape]{scala}
def hopCount(): Int = 
...
\end{minted}
\mintinline{scala}{recentValues(data, k)} is an aggregate operator that returns a list containing the last \textbf{\texttt{k}} data perceived by the agent.
...
The idea is to maintain the nodes in a stable situation or to
 speed up the time needed to reach a stable one.
%
...
\begin{figure}
  ...
  Then, an agent evaluates the action to speed up the hop count convergence following a policy. 
...
We use 80 ($N$) nodes displaced into an irregular grid with a width of 100 meters and a height of 20 meters. 
 Each node has a neighbourhood range equal to 8 meters --- on average, each node has 7 neighbours. 
...
The aggregate speed-up convergence is observed for each scenario taken into consideration.
...
The application of RL to this kind of setting is a largely unexplored %-- even if some works exists~\cite{DBLP:conf/icml/YangLLZZW18} -- 
 research area. There are a variety of challenges, such as non-stationarity,  partial environment observability, and non-bounded system size.
In a case study, we have empirically shown the effectiveness of RL by showing a speedup of the convergence time of
 the aggregate program we took into consideration, namely the hop count.
...
To the best of our knowledge, this is the first effort towards a ``hybrid'' collective programming approach, where the high-level program specification is 
 done by a designer and the low-level implementation to building blocks based on machine learning.
%
As such, this work has shed light on several issues deserving future investigation.
...
Aggregate systems are partially observable from the perspective of each node. 
 ...
 Learning algorithms were implemented at the building block level. 
 Another possible direction is to use them at the framework level, e.g., to select which messages to send to the neighbourhood or not. 
...
In our experiments, we have applied a variation of the independent learning approach. 
...
Finally, we do not address the multi-agent credit assignment problem in this work. 
 Indeed, we use a global reward function for the whole aggregate. In this way, however, learning often degenerated because it was difficult to understand the influence of an agent on the system. 
 To reduce this problem, we plan to study the application of difference rewards~\cite{DBLP:conf/atal/AgoginoT04} or counterfactual baselines~\cite{DBLP:journals/corr/FoersterFANW17}.

\bibliographystyle{IEEEtran}
\bibliography{biblio-reduced}
%%
